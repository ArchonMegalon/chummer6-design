...
| `chummer-android`          | native Android product head        | native MAUI Shell, Android adapters, native Build/Play/Campaign projections, package/signing recipes, device tests, Play evidence | copied rules/session truth, PWA/WebView product surfaces, hosted identity, release authority, provider execution | consumes `Chummer.Presentation`, `Chummer.Engine.Contracts`, `Chummer.Play.Contracts`, `Chummer.Campaign.Contracts`, `Chummer.Ui.Kit` |
