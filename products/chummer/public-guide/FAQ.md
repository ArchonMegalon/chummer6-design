...
- **What platforms are publicly available today?** Windows and Linux are the current public path; check Download for exact status.
...
Yes. Start with Download and Status: the current promoted shelf names the supported platforms, while each optional campaign tool keeps its own availability label.
