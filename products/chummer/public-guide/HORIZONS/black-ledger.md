...
- Today: signed in command lane live.
- Next: limited coaching expansion.

## Explanation videos

* [Watch the BLACK LEDGER 90-second deep dive](https://chummer.run/media/horizons/black-ledger-90s-deepdive.mp4).
* [Watch the BLACK LEDGER epic reel](https://chummer.run/media/horizons/black-ledger-epic-90s.mp4).
...
BLACK LEDGER is not only a map and a board. It also has public-safe faction promo paths that show how each banner sells itself to the city.
...
Each promo path is supposed to feel like a recruitment bulletin with records, not a soft teaser:
...
The newsroom is not allowed to behave like a generic motion-card teaser. It must stay downstream of kept in Chummer records and, at flagship quality, ship as a real broadcast-style bulletin with:
...
The accepted newsroom bar lives in:
...
That is the line BLACK LEDGER tries to hold everywhere: it is allowed to look dramatic, but it is not allowed to outrun evidence.
...
> **User lore is fuel. Chummer still requires review before it becomes world state.**
...
BLACK LEDGER can integrate with Table Pulse — carefully, and with the path split kept explicit.

Table Pulse Live is not live surveillance.
Table Pulse Aftermath is not player scoring.
Neither path is moderation state.
Neither path is automatic world state.
...
Table Pulse Live is a separate path. It handles in-world heat packets, limited remote reactions,
...
Table Pulse and GOD Observer require consent and do not automatically create world state.
...
The first evidence should be small and strong:
