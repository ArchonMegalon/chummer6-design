...
12. Visual familiarity is proven, not assumed.
   ...
   - SR5 flagship claims must also clear `SR5_FLAGSHIP_VISUAL_PARITY_EXIT.yaml`, which starts with four blocking workflows: new-character opening, attributes, skills, and cyberware add/edit.
   - `SR5_FIRST_MINUTE_TASKS.yaml` is the executable SR5 veteran-orientation contract for the first-minute proof lane, and `SR5_SCREENSHOT_ORACLE_REGISTRY.yaml` defines the same-stage Chummer5a screenshot oracle pairs for visual review.
...
### 5b. SR5 veteran visual parity lane

For SR5 flagship claims, the promoted desktop head must also satisfy `SR5_FLAGSHIP_VISUAL_PARITY_EXIT.yaml`.

That lane is release-blocking and must prove:

- same-stage screenshot-oracle comparison against Chummer5a as defined in `SR5_SCREENSHOT_ORACLE_REGISTRY.yaml`
- passing first-minute veteran task proof as defined in `SR5_FIRST_MINUTE_TASKS.yaml`
- no debug, payload, or internal-inspector posture on default flagship screenshots
- center-dominant dense workbench posture on the four blocking SR5 workflows before broader parity claims are treated as trustworthy
...
- If the SR5 flagship visual parity exit or first-minute task receipt is missing, stale, or failing for the promoted SR5 head, the gate fails.
...
- If a default flagship SR5 screen shows a debug, payload, or internal inspector panel, the gate fails.
- If top chrome grows large enough to crowd out the center builder or the bottom status posture disappears, the gate fails.
