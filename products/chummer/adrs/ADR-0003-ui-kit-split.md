...
## Context
- The split order calls for `chummer-ui-kit` immediately after the play split.
...
## Decision
- `chummer-ui-kit` is the package-only shared UI boundary for presentation and play.
...
- Shared visual primitives must move into `chummer-ui-kit` instead of being duplicated between presentation and play.
