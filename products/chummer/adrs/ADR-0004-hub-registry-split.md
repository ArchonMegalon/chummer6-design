...
## Context
- The split order places `chummer-hub-registry` after `chummer-ui-kit`.
...
## Decision
- `chummer-hub-registry` becomes the canonical owner for immutable artifact catalog data, publication workflow state, moderation state, installs, reviews, and runtime-bundle head metadata.
- `chummer.run-services` retains hosted orchestration, identity, session relay, Spider, approvals, and play API aggregation, but sheds registry persistence internals as the split lands.
...
- The later `chummer-media-factory` split stays viable because hub-registry owns bundle heads and catalog truth, not rendering internals.
