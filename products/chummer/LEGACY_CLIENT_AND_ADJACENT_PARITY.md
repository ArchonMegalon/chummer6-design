...
| Sourcebooks, source toggles, rule snippets, master index, PDF/URL reference posture | `Edit Sourcebook Info`, `Sourcebooks to Use`, `MasterIndex`, PDF opening, reference snippets | master-index sourcebook metadata, snippet coverage posture, and governed stale/missing reporting are now explicit; first-class source selection UX and approved PDF/URL snapshot governance still need closure | partial | `13` |
| Settings and authored rules-environment posture | global settings, character settings, source toggles, rule-environment visibility | master-index settings lane now projects profile counts, toggle coverage, and stale-vs-governed source-toggle posture from `settings.xml`; first-class settings UX consolidation is still open | partial | `14` |
| Custom data, XML editing, translator tooling, amend authoring bridge | `EditXmlData`, translator-era XML corpus, custom descriptions/data | master-index now projects custom-data directory counts plus missing/stale/governed custom-data lane posture from `settings.xml` + enabled overlays; end-user custom-data authoring UX and translator replacement workflow remain open | partial | `14` |
