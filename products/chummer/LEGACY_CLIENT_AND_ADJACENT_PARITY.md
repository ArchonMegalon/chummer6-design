...
| Sourcebooks, source toggles, rule snippets, master index, PDF/URL reference posture | `Edit Sourcebook Info`, `Sourcebooks to Use`, `MasterIndex`, PDF opening, reference snippets | modern sourcebook and rules-reference lane is not explicit enough today | missing | `13` |
| Settings and authored rules-environment posture | global settings, character settings, source toggles, rule-environment visibility | `RULE_ENVIRONMENT_AND_AMEND_SYSTEM.md`, `DESKTOP_CLIENT_PRODUCT_CUT.md` | partial | `14` |
| Custom data, XML editing, translator tooling, amend authoring bridge | `EditXmlData`, translator-era XML corpus, custom descriptions/data | `RULE_ENVIRONMENT_AND_AMEND_SYSTEM.md`, `LOCALIZATION_AND_LANGUAGE_SYSTEM.md` | partial | `14` |
