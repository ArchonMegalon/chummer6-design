...
| WL-D007 | blocked | P1 | Publish review-guidance template mirrors into each code repo under `.codex-design/review/REVIEW_CONTEXT.md` and record per-repo publish evidence. | agent | Fresh rerun at `2026-03-11T17:17:29Z` confirmed WL-D007-07 remains blocked because `/docker/chummercomplete/chummer-media-factory` is still not provisioned; WL-D007-01..06 remain blocked by sandbox `Permission denied` on sibling-repo `.codex-design/review` republish writes (latest refs: core `b4a782d2`, presentation `5962b1ce`, run-services `a3d0697f`, play `f1340928`, ui-kit `98ae0873`, hub-registry `2bcf6955`). |
