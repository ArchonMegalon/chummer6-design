...
def test_validate_public_guide_editorial_covers_rejects_unapproved_kind_exception(
    tmp_path: Path, monkeypatch
) -> None:
    assert validator.Image is not None
        ...
        yaml.safe_dump(
            {
                ...
                "assets": {
                    asset_rel: {
                        "kind": "comic_panel_cover",
                        "series_label": "Campaign OS",
                        "title": "Test cover",
                        "subtitle": "Unapproved kind drift",
                    }
                },
            },
            sort_keys=False,
    ...
    try:
        ...
        raise AssertionError("validator.main() should have failed for an unapproved kind exception")

    assert "kind must stay 'feature_cover'" in message
